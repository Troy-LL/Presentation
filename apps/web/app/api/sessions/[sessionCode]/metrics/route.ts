...
import { getSessionMetrics } from "@/lib/party-admin";
...
export async function GET(
  ...
  try {
    const payload = await getSessionMetrics(sessionCode.toUpperCase(), hostToken);
    return NextResponse.json(payload);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not fetch session metrics.";
